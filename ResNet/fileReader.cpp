...
void fileReader::load_cn_weights() {
   ...
   for (unsigned iter = 0; iter < trained_iter; iter++) {
            std::cout << "Iteration " << iter + 1 << " weights:" << std::endl;
            for (unsigned i = 0; i < M; i++) {
                std::cout << "Node " << i + 1 << ": ";
                for (unsigned j = 0; j < dc[i]; j++) {
                    std::cout << std::setw(10) << std::fixed << std::setprecision(6) << weights_cn[iter][i][j] << " ";
                }
                std::cout << std::endl;
            }
            std::cout << std::endl;
...
// Add this include for printing debug information

void fileReader::load_ri_weights() {
    auto path = construct_weights_path("weight_ri.txt");
    std::vector<std::vector<std::vector<double>>> weight_ri;
    std::string line;
    std::ifstream weights_file;
    weights_file.open(path.c_str());
    unsigned dec_iter;
    ...
    // rest of the lines, value of each rows of all iterations
    for (unsigned iter = 0; iter < dec_iter; iter++) {
        weight_ri.emplace_back();
    }
    for (unsigned iter = 0; iter < dec_iter; iter++) {
        std::vector<std::vector<double>> weight_ri_tmp;
        for (unsigned i = 0; i < m; i++) {
            std::vector<double> weight_ri_row(dc[i], 0);
            for (unsigned j = 0; j < dc[i] - 1; j++) {
                getline(weights_file, line, ' ');
                weight_ri_row[j] = std::stod(line);
            }
            getline(weights_file, line);
            weight_ri_row[dc[i] - 1] = std::stod(line);
            weight_ri_tmp.push_back(weight_ri_row);
        }
        weights_ri.push_back(weight_ri_tmp);
    }
